...
def analyze_trend(df):
    ...
    score = 0  # 趋势评分（正数看多，负数看空）

    # 1. EMA趋势：EMA12 > EMA26 看多，加1分；否则减1分
    if latest_ema12 > latest_ema26:
        score += 1
    else:
        score -= 1

    # 2. MACD趋势：MACD > 信号线 看多，加1分；否则减1分
    if latest_macd > latest_signal:
        score += 1
    else:
        score -= 1

    # 3. 价格位置：收盘价 > EMA12 看多，加1分；否则减1分
    ...
    # 根据评分判断最终趋势
    if score >= 3:
        return "🟢 强多"  # 满分3分：强多头趋势
    elif score == 2:
        return "🟡 偏多"  # 2分：偏多头趋势
    elif score <= -3:
        return "🔴 强空"  # 满分-3分：强空头趋势
    elif score == -2:
        return "🟠 偏空"  # -2分：偏空头趋势
    else:
        return "⚪ 震荡"  # 其他：震荡行情
...
def on_message(ws, message):
    ...
    # ✅ 关键修复：解析combined stream（多流合并推送的数据格式）
...
def monitor():
    ...
    while True:
        ...
        if range_low and range_high:
            near_low = price <= range_low * 1.001  # 价格 <= 低点*1.001
            near_high = price >= range_high * 0.999  # 价格 >= 高点*0.999
